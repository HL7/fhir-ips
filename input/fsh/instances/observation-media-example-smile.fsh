Instance: media-example-smile
InstanceOf: Observation
Title: "Observation: Media example"
Description: "Observation: Media example"
Usage: #example
* status = #final
//* status = #completed
* code = $media-type#image "Image" // used in place of Media.type in R4
// could also consider using 'category' in some capacity for this?
//* type = $media-type#image "Image"
* subject = Reference(Patient/eumfh-39-07)
* effectiveDateTime = "2017-12-17"
* issued = "2017-12-17T14:56:18Z"
* performer = Reference(Practitioner/eumfh-39-07)
* valueAttachment.height = 128
* valueAttachment.width = 128
* valueAttachment.id = "a1"
* valueAttachment.contentType = #image/png
* valueAttachment.data = ""
* valueAttachment.creation = "2017-12-17"