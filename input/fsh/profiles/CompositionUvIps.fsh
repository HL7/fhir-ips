...
* attester.party MS
* custodian MS
* relatesTo.resourceReference only Reference(Composition or CompositionUvIps)
...
* event[careProvisioningEvent].detail.concept 1..1 MS
* event[careProvisioningEvent].detail.concept = $v3-ActClass#PCPR
...
* section[sectionMedicalDevices].entry only Reference(DeviceUsage or DocumentReference)
...
* section[sectionMedicalDevices].entry[deviceStatement] only Reference(DeviceUsageUvIps)
...
* section[sectionResults].entry[results-media] only Reference(ObservationMediaUvIps)
