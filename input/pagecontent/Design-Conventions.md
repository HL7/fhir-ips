...
<p>If an [IPS Producer](./ActorDefinition-IPS-Producer.html) does not have data to be included in the IPS, the data element is omitted.</p>
<p>Note: an [IPS Producer](./ActorDefinition-IPS-Producer.html) may have no data to be included in the IPS either because there are no data, data are not allowed to be shared or because data available are not pertinent with the scope of the IPS.</p>

#### Required MustSupport Data Elements (cardinality of 1..1 or 1..\*)

<p>If an [IPS Producer](./ActorDefinition-IPS-Producer.html) does not have data to be included in the IPS, the reason for the absence has to be specified as follows:</p>
